...
export class EventCategoryService {
  ...
  async addCategory(name: string) {
    const exists = await this.eventCategoryModel.exists({
    ...
    if (!category) {
      throw new BadRequestException(
        "Cannot create category now. Please try again later"
      );
    }
  ...
  async deleteCategory(name: string) {
    console.log(name);
    const { deletedCount } = await this.eventCategoryModel.deleteOne({
      name: name.toLowerCase(),
    });
